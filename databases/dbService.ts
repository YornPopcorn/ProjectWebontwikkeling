...
export async function getGuitarsByBrandId(brandId: string): Promise<Guitar[]> {
	return await guitarsCollection
		.find({
			$or: [{ "brand.id": parseInt(brandId) }, { brandId: parseInt(brandId) }],
		})
		.toArray();
}
...
export async function getBrandsWithCounts(): Promise<Array<Brand & { guitarCount: number }>> {
	...
	return brands.map((brand) => {
		const count = guitars.filter((g) => g.brand?.id === brand.id || g.brand?.brandName === brand.brandName).length;
...
export async function getFilteredAndSortedBrands(
	...
	if (filter) {
		query.$or = [{ brandName: { $regex: filter, $options: "i" } }, { country: { $regex: filter, $options: "i" } }];
	}
